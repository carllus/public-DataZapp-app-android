...
class ResourceLoader implements Runnable {
    ...
    private boolean okToLoad() {
        ...
        Log.m546v("...no network connectivity");
        return false;
    }
    ...
    private void loadResource() {
        ...
        Log.m546v("Start loading resource from network ...");
        ...
        try {
            ...
            try {
                ...
                Log.m546v("Successfully loaded supplemented resource: " + resource);
                if (resource.resource == null) {
                    Log.m546v("No change for container: " + this.mContainerId);
                }
                this.mCallback.onSuccess(resource);
                networkClient.close();
                Log.m546v("Load resource from network finished.");
            } catch (IOException e) {
                Log.m549w("Error when parsing downloaded resources from url: " + url + " " + e.getMessage(), e);
                this.mCallback.onFailure(LoadCallback.Failure.SERVER_ERROR);
            }
        } catch (FileNotFoundException e2) {
            Log.m548w("No data is retrieved from the given url: " + url + ". Make sure container_id: " + this.mContainerId + " is correct.");
            this.mCallback.onFailure(LoadCallback.Failure.SERVER_ERROR);
        } catch (IOException e3) {
            Log.m549w("Error when loading resources from url: " + url + " " + e3.getMessage(), e3);
    ...
    public void setCtfeURLPathAndQuery(String urlPathAndQuery) {
        ...
        Log.m540d("Setting CTFE URL path: " + urlPathAndQuery);
    ...
    public void setPreviousVersion(String version) {
        Log.m540d("Setting previous container version: " + version);
