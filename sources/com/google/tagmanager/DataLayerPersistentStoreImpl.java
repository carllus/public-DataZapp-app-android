...
class DataLayerPersistentStoreImpl implements DataLayer.PersistentStore {
    ...
    private void makeRoomForEntries(int count) {
        ...
        if (entrysOverLimit > 0) {
            ...
            Log.m544i("DataLayer store full, deleting " + entrysToDelete.size() + " entries to make room.");
    ...
    public void clearKeysWithPrefixSingleThreaded(String keyPrefix) {
        ...
        if (db != null) {
            try {
                Log.m546v("Cleared " + db.delete(MAPS_TABLE, "key = ? OR key LIKE ?", new String[]{keyPrefix, keyPrefix + ".%"}) + " items");
            } catch (SQLiteException e) {
                Log.m548w("Error deleting entries with key prefix: " + keyPrefix + " (" + e + ").");
            } finally {
                closeDatabaseConnection();
            }
        }
    ...
    private void deleteEntriesOlderThan(long timeInMillis) {
        ...
        if (db != null) {
            try {
                Log.m546v("Deleted " + db.delete(MAPS_TABLE, "expires <= ?", new String[]{Long.toString(timeInMillis)}) + " expired items");
            } catch (SQLiteException e) {
                Log.m548w("Error deleting old entries.");
            }
        }
    }
    ...
    private void deleteEntries(String[] entryIds) {
        ...
        if (entryIds != null && entryIds.length != 0 && (db = getWritableDatabase("Error opening database for deleteEntries.")) != null) {
            try {
                ...
                Log.m548w("Error deleting entries " + Arrays.toString(entryIds));
            }
        }
    }
    ...
    private List<String> peekEntryIds(int maxEntries) {
        ...
        if (maxEntries <= 0) {
            Log.m548w("Invalid maxEntries specified. Skipping.");
        } else {
            ...
            if (db != null) {
                ...
                try {
                    ...
                    Log.m548w("Error in peekEntries fetching entryIds: " + e.getMessage());
                    if (cursor != null) {
                        cursor.close();
                    }
                } catch (Throwable th) {
    ...
    private int getNumStoredEntries() {
        ...
        try {
            ...
            Log.m548w("Error getting numStoredEntries");
            if (cursor != null) {
                cursor.close();
            }
        } catch (Throwable th) {
