...
class DelayedHitSender implements HitSender {
    ...
    public boolean sendHit(String url) {
        if (!this.mRateLimiter.tokenAvailable()) {
            Log.m548w("Too many urls sent too quickly with the TagManagerSender, rate limiting invoked.");
            return false;
        }
        if (!(this.mWrapperUrl == null || this.mWrapperQueryParameter == null)) {
            try {
                ...
                Log.m546v("Sending wrapped url hit: " + url);
            } catch (UnsupportedEncodingException e) {
                Log.m549w("Error wrapping URL for testing.", e);
                return false;
            }
        }
