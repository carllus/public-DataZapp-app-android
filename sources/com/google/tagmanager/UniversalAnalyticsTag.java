...
class UniversalAnalyticsTag extends TrackingTag {
    ...
    /* renamed from: ID */
    private static final String f723ID = FunctionType.UNIVERSAL_ANALYTICS.toString();
    ...
    public void evaluateTrackingTag(Map<String, TypeSystem.Value> tag) {
        ...
        if (checkBooleanProperty(tag, ANALYTICS_PASS_THROUGH)) {
            ...
            Log.m548w("Ignoring unknown tag.");
        }
    ...
    private void sendTransaction(Tracker tracker, Map<String, TypeSystem.Value> tag) {
        ...
        if (transactionId == null) {
            Log.m542e("Cannot find transactionId in data layer.");
            return;
        }
        ...
        try {
            ...
            if (items != null) {
                for (Map<String, String> item : items) {
                    if (item.get("name") == null) {
                        Log.m542e("Unable to send transaction item hit due to missing 'name' field.");
                        return;
                    }
            ...
            Log.m543e("Unable to send transaction", e);
        }
    }
